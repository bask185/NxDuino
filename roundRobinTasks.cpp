...
//#define DEBUG CANNOT USE SERIAL PORT BECAUSE OF XPRESSNET LIBRARY.... 
...
void debug( int num )
{
 ...
 // can't use bloody serial port because Xnet... should do something with dem status led
void loadSDcard()
{
    //debug(F("\r\nloading rail map from SD card\r\n")) ;

   if( !SD.begin( CSpin ) )  return ; //debug(F("\r\nSD card initialized\r\n")) ;
...
void loadEEPROM(uint16_t *iodir){ // returns ammount of requied MCP23017 slaves depending on taught in IO
    ...
    for( uint8_t element = 0; element < 128 ; element++ ) 
    {
        ...
        if( _IO.type >= start_stop_sw && _IO.type  <= occupancy_2_I2C )     // if device is I2C input,
        {
            ...
            *iodir |= 0x01 << pin;                             // configure pin as input pullup
